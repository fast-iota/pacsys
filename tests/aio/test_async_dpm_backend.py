...
class TestAsyncDPMRead:
    ...
    @pytest.mark.asyncio
    async def test_get_many_shares_one_budget_across_connect_and_read(self, monkeypatch):
        """Connect time is charged against the call timeout; read_many gets only the remainder."""
        now = [100.0]
        monkeypatch.setattr("pacsys.aio._dpm_http.time.monotonic", lambda: now[0])
        ...
        try:
            await backend.get_many(["M:OUTTMP"], timeout=1.0)
            assert budgets == [0.375]
        finally:
            await backend.close()
...
class TestAsyncDPMWrite:
    ...
    async def test_write_connect_timeout_respects_call_deadline(self):
        ...
        backend._create_core = slow_create
        try:
            results = await asyncio.wait_for(backend.write_many([("M:OUTTMP", 72.5)], timeout=0.2), timeout=1.0)
            assert cancelled.is_set()
