...
def _binary_search(
    ...
    for _ in range(_MAX_SEARCH_ITER):
        ...
        if cumid_val == target:
            return pumid
        if cumid_val > target and cuupr_val >= target or cumid_val < target and cuupr_val <= target:
            puupr = pumid
        elif cumid_val < target and cuupr_val >= target or cumid_val > target and cuupr_val <= target:
            pulow = pumid
