...
#
# Local, non-generated additions (the generator is no longer run; keep these if it ever is):
#   _Cursor and the cursor branches in consumeRawInt/unmarshal_double/unmarshal_string/
#   unmarshal_binary/unmarshal_array (bulk NumPy decode of double arrays), and _seq_eq.
...
class _Cursor:
    """Byte iterator that also knows its offset, so decoders can slice the buffer.

    Drop-in for ``iter(data)`` (works with ``__next__``/``islice``); the cursor
    branches below use ``take()`` for bulk reads. Feeding a plain iterator keeps
    the original byte-at-a-time path.
    """
...
_DOUBLE_REC = None  # numpy dtype for one SDD double (tag 0x28 + big-endian f8); built lazily
...
class ScalarArray_reply:
    ...
    def __eq__(self, other):
        return (
            ...
            and self.cycle == other.cycle
            and self.status == other.status
            and _seq_eq(self.data, other.data)
        )
...
def consumeRawInt(it, tag):
    if isinstance(it, _Cursor):
        buf, pos = it.buf, it.pos
        if pos >= len(buf):
            raise ProtocolError("truncated message")
        itTag = buf[pos]
        itLen = itTag & 0xF
        if (itTag & 0xF0) != tag or not 0 < itLen <= 8:
            raise ProtocolError("bad tag or length")
        end = pos + 1 + itLen
        if end > len(buf):
            raise ProtocolError("truncated message")
        it.pos = end
        return int.from_bytes(buf[pos + 1 : end], "big", signed=True)
    itTag = it.__next__()
    itLen = itTag & 0xF
...
def unmarshal_double(it):
    raw = it.take(9) if isinstance(it, _Cursor) else bytes(islice(it, 9))
    if raw[0] == 40:
        (v,) = _unpack_float(raw)
        return v
    else:
...
def unmarshal_binary(ii):
    n = consumeRawInt(ii, 0x30)
    return bytes(ii.take(n)) if isinstance(ii, _Cursor) else bytes(islice(ii, n))


def unmarshal_array(ii, fn):
    n = consumeRawInt(ii, 0x50)
    if fn is unmarshal_double and isinstance(ii, _Cursor):
        # Doubles are fixed 9-byte records: decode the whole array in one numpy pass
        global _DOUBLE_REC
        import numpy as np

        if _DOUBLE_REC is None:
            _DOUBLE_REC = np.dtype([("tag", "u1"), ("v", ">f8")])
        rec = np.frombuffer(ii.take(9 * n), dtype=_DOUBLE_REC, count=n)
        if n and not (rec["tag"] == 40).all():
            raise ProtocolError("expected tag for double")
        return rec["v"].astype("=f8")  # one vectorized copy -> native-endian, owning array
    return [fn(ii) for x in range(n)]
